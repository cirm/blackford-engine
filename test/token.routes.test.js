...
describe('routes: /token', () => {
  test('generate valid token', async () => {
    ...
    const response = await fetch(`${server}/api/token`, { method: 'POST', body: JSON.stringify(body), headers }).then(res => res.json());
  ...
  test('renew valid token', async () => {
    const body = { username: 'bakufu', password: 'EndOria' };
    const { apiToken } = await fetch(`${server}/api/token`, { method: 'POST', body: JSON.stringify(body), headers }).then(res => res.json());
    const response = await fetch(`${server}/api/token`, { method: 'PUT', body: JSON.stringify({ apiToken }), headers }).then(res => res.json());
  ...
  test('renew missing token', async () => {
    const response = await fetch(`${server}/api/token`, { method: 'PUT', body: JSON.stringify({ }), headers }).then(res => res.json());
    expect(response).toEqual({ error: 'Missing or malformed Token', status: 401 });
  });
});
...
describe('routes: /gameToken', () => {
  test('generate valid mobile token', async () => {
    const body = { username: 'bakufu', password: 'EndOria' };
    const response = await fetch(`${server}/api/gameToken`, { method: 'POST', body: JSON.stringify(body), headers }).then(res => res.json());
