...
describe('routes: /token', () => {
  test('generate valid token', async () => {
    ...
    const response = await fetch(`${server}/token`, { method: 'POST', body: JSON.stringify(body), headers: { 'Content-Type': 'application/json' } }).then(res => res.json());
  ...
  test('wrong password', async () => {
    ...
    const response = await fetch(`${server}/token`, { method: 'POST', body: JSON.stringify(body), headers: { 'Content-Type': 'application/json' } }).then(res => res.json());
    expect(response).toEqual({ error: 'Wrong username or password', code: 401 });
  });
  ...
  test('renew valid token', async () => {
    const body = { username: 'bakufu', password: 'EndOria' };
    const { apiToken } = await fetch(`${server}/token`, { method: 'POST', body: JSON.stringify(body), headers: { 'Content-Type': 'application/json' } }).then(res => res.json());
    const response = await fetch(`${server}/token`, { method: 'PUT', body: JSON.stringify({ apiToken }), headers: { 'Content-Type': 'application/json' } }).then(res => res.json());
  ...
  test('renew invalid token', async () => {
    ...
    const response = await fetch(`${server}/token`, { method: 'PUT', body: JSON.stringify({ apiToken }), headers: { 'Content-Type': 'application/json' } }).then(res => res.json());
    expect(response).toEqual({ error: 'Missing or malformed Token', code: 401 });
  });
  ...
  test('renew missing token', async () => {
    const response = await fetch(`${server}/token`, { method: 'PUT', body: JSON.stringify({ }), headers: { 'Content-Type': 'application/json' } }).then(res => res.json());
    expect(response).toEqual({ error: 'Missing or malformed Token', code: 401 });
  });
});
...
describe('routes: /gameToken', () => {
  test('generate valid mobile token', async () => {
    const body = { username: 'bakufu', password: 'EndOria' };
    const response = await fetch(`${server}/gameToken`, { method: 'POST', body: JSON.stringify(body), headers: { 'Content-Type': 'application/json' } }).then(res => res.json());
